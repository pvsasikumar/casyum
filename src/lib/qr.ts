...
export function normalizeCasyumQrValue(rawValue: unknown): string | null {
  ...
  // unbounded (the old recursive call overflowed the stack and the scan
  // handler surfaced it as a bogus "Invalid QR Code").
  let previous = '';
  while (previous !== raw) {
    const lower = raw.toLowerCase();
    previous = raw;
 ...
 *   - `CASYUM:CAS-02`
 *   - `CASYUM:PARTICIPANT:CAS-02` (current canonical payload)
