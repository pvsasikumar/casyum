...
const NAV_ITEMS = [
  { to: '/observer/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { to: '/observer/events', label: 'All Events', icon: Calendar },
  { to: '/observer/participants', label: 'Participants', icon: Users },
  { to: '/observer/payments', label: 'Payments', icon: CreditCard },
  { to: '/observer/attendance', label: 'Attendance', icon: UserCheck },
...
export const ObserverLayout: React.FC = () => {
  const { user, isAuthenticated, isLoading, logout } = useObserver();
  ...
  useEffect(() => {
    document.title = `CASYUM Observer — ${pageTitle}`;
  }, [pageTitle]);
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-violet-500/30 selection:text-violet-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold font-display text-white text-sm leading-none">
                CASYUM <span className="text-violet-400">Observer</span>
              </span>
      ...
      {/* READ ONLY MODE banner — shown on every page */}
      <ReadOnlyBanner />

      {/* Breadcrumb */}
      <div className="border-b border-white/5 bg-black/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-2 text-[11px]">
          <span className="text-white/40">Observer</span>
          <ChevronRight className="w-3 h-3 text-white/30" />
          <span className="text-amber-400 font-bold">{pageTitle}</span>
        </div>
      </div>
      ...
      {logoutDialogOpen && (
        <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-zinc-950 border border-white/20 rounded-3xl p-6 shadow-2xl flex flex-col gap-4">
            ...
            <p className="text-sm text-white/70">Are you sure you want to logout from the Observer Portal?</p>
