...
export const ParticipantsPage: React.FC = () => {
  const { participants, participantsLoading } = useObserver();
    ...
    return participants.filter((p) => {
      ...
      if (
        ...
        !(
          ...
          String(p.casyumId || '').toLowerCase().includes(q)
        )
      ) {
        return false;
      }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-white min-w-[980px]">
            ...
            <tbody className="divide-y divide-white/5">
              {participantsLoading ? (
                <tr>
                  <td colSpan={9} className="p-12 text-center text-xs text-white/40">
                    <Loader2 className="w-5 h-5 animate-spin mx-auto text-violet-400" />
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} className="p-12 text-center text-xs text-white/40">No participants found.</td>
                </tr>
              ) : (
                filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="p-4 font-mono text-[10px] text-amber-300">{p.casyumId || '—'}</td>
                      <td className="p-4">
