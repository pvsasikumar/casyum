...
export const DashboardPage: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const totalPayments = registrations.filter(
      (r) => r.paymentStatus && String(r.paymentStatus).toLowerCase() !== 'not_required'
    ).length;
    ...
    return { totalPayments, submitted, verified, rejected, present, absent, revenue };
  }, [registrations, attendance]);

  const cards: CardDef[] = [
    { label: 'Total Participants', value: participants.length, sub: 'Registered participants', icon: Users, tone: 'violet' },
    { label: 'Total Registrations', value: registrations.length, sub: 'All event registrations', icon: ClipboardList, tone: 'indigo' },
    { label: 'Total Events', value: events.length, sub: 'Across all CASYUM tracks', icon: Calendar, tone: 'cyan' },
    { label: 'Total Coordinators', value: coordinators.length, sub: 'Event coordinators', icon: UserCog, tone: 'sky' },
    { label: 'Faculty Management', value: faculty.length, sub: 'CASYUM Faculty members', icon: GraduationCap, tone: 'indigo' },
    { label: 'Registration Team', value: registrationTeam.length, sub: 'Desk staff members', icon: IdCard, tone: 'sky' },
    { label: 'Payments Submitted', value: stats.totalPayments, sub: 'All payment submissions', icon: IndianRupee, tone: 'amber' },
    { label: 'Payments Verified', value: stats.verified, sub: 'Payment proof confirmed', icon: ShieldCheck, tone: 'emerald' },
    { label: 'Pending Payments', value: stats.submitted, sub: 'Awaiting verification', icon: Hourglass, tone: 'rose' },
    { label: 'Total Attendance', value: attendance.length, sub: 'Attendance records', icon: Activity, tone: 'violet' },
    { label: 'Present Participants', value: stats.present, sub: 'Marked present', icon: BadgeCheck, tone: 'emerald' },
    { label: 'Absent Participants', value: stats.absent, sub: 'Marked absent', icon: UserCheck, tone: 'rose' },
  ];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8 select-none pb-16">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold font-display text-white tracking-tight flex items-center gap-3">
          CASYUM <span className="text-violet-400">Observer</span> Portal
          <ReadOnlyBadge />
        </h1>
        <p className="text-xs text-white/50">
          Live read-only view of participants, registrations, payments, attendance and results.
        </p>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-white min-w-[760px]">
              ...
              <tbody className="divide-y divide-white/5">
                {recent.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-10 text-center text-xs text-white/40">
                    ...
                    return (
                      <tr key={r.registration_id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="p-4 font-mono font-extrabold text-emerald-400">
                          {r.registrationFee ? formatRupees(Number(r.registrationFee) || 0) : '—'}
                        </td>
                        <td className="p-4">
