...
import { Calendar, Search, Loader2, MapPin, Clock, IndianRupee, Eye } from 'lucide-react';
...
export const EventsPage: React.FC = () => {
  const { events, eventsLoading, registrations } = useObserver();
  ...
  const rows = useMemo(() => {
    ...
    return events
      ...
      .map((e) => {
        ...
        return { ...e, regs: regs.length, verified, pending, revenue };
      })
                ...
                rows.map((e) => {
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="p-4">
                        <Link
                          to={`/observer/events/${e.id}`}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-[10px] font-bold text-white/80 hover:text-white hover:bg-white/10 transition-all cursor-pointer"
                        >
