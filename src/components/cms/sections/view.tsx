  ...
  <div className="rounded-3xl overflow-hidden border border-white/10">
    {hero.bannerImage && (
      <div className="relative">
        <img src={hero.bannerImage} alt={hero.title} className="w-full aspect-video object-cover" />
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {items.map((item) => (
        <div key={item.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3">
          {item.logoUrl ? (
            <img src={item.logoUrl} alt={item.name} className="w-10 h-10 rounded-lg object-contain bg-white/5 border border-white/10 p-1 flex-shrink-0" />
          ) : (
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className={`${content.aspect || 'aspect-video'} w-full overflow-hidden rounded-xl bg-white/5 border border-white/10`}>
        <img src={content.url} alt={content.alt || content.caption || 'Image'} className="w-full h-full object-cover" loading="lazy" />
      </div>
