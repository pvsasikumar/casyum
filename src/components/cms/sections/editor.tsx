...
export const SectionEditor: React.FC<{
  ...
  switch (section.sectionType) {
    ...
    case 'gallery':
      return (
        <CmsItemList<CmsGalleryItem>
          ...
          renderEditor={(item, update) => (
            <>
              <CmsImageField value={item.url} onChange={(url) => update({ url })} storagePrefix={`cms/${eventId}/gallery`} canEdit />
              <div className="flex gap-2">
          ...
          renderView={(item) => (
            <div className="rounded-xl overflow-hidden border border-white/10 bg-white/[0.03]">
              <img src={item.url} alt={item.caption || item.category} className="w-full aspect-video object-cover" loading="lazy" />
            </div>
          )}
        />
      );
        ...
        <CmsItemList<CmsSponsor>
          ...
          renderEditor={(item, update) => (
            <>
              ...
              <CmsImageField value={item.logoUrl} onChange={(url) => update({ logoUrl: url })} storagePrefix={`cms/${eventId}/sponsors`} aspect="aspect-video" canEdit />
    ...
    case 'image':
      return (
        <div className="flex flex-col gap-4">
          <CmsImageField value={section.content.url} onChange={(url) => onChange({ url })} storagePrefix={`cms/${eventId}/images`} canEdit />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
const HeroEditor: React.FC<{ content: CmsHero; eventId: string; onChange: (patch: Partial<CmsHero>) => void }> = ({ content: hero, eventId, onChange }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <CmsImageField value={hero.bannerImage} onChange={(url) => set({ bannerImage: url })} storagePrefix={`cms/${eventId}/hero`} label="Event Banner" canEdit />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
const CmsFileField: React.FC<{ value: string; onChange: (url: string) => void; storagePrefix: string }> = ({ value, onChange, storagePrefix }) => {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const url = await uploadFileToStorage(path, file, { contentType: file.type || 'application/octet-stream' });
      onChange(url);
    } catch {
