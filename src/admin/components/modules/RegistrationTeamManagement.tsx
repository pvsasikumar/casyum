...
import {
  ...
  createRegistrationTeamMember,
  updateRegistrationTeamMember,
  setRegistrationTeamStatus,
  deleteRegistrationTeamMember,
  resetRegistrationTeamPassword,
  listRegistrationTeamActivity,
  type RegistrationTeamMember,
  type RegistrationTeamActivity,
} from '../../../services/registrationTeamService';
...
const ACTIVITY_ACTION_STYLES: Record<string, string> = {
  'Member Created': 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30',
  'Member Updated': 'bg-sky-500/20 text-sky-300 border-sky-500/30',
  'Member Enabled': 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30',
  'Member Disabled': 'bg-amber-500/20 text-amber-300 border-amber-500/30',
  'Member Deleted': 'bg-rose-500/20 text-rose-300 border-rose-500/30',
...
export const RegistrationTeamManagement: React.FC = () => {
  const rbac = useRBAC();
  const { addToast } = useAdmin();
  const [tab, setTab] = useState<'members' | 'activity'>('members');
  const [members, setMembers] = useState<RegistrationTeamMember[]>([]);
  ...
  const [editing, setEditing] = useState<RegistrationTeamMember | null>(null);
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<RegistrationTeamMember | null>(null);
  const [statusTarget, setStatusTarget] = useState<RegistrationTeamMember | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);
  const [resetTarget, setResetTarget] = useState<RegistrationTeamMember | null>(null);
  ...
  const [viewTarget, setViewTarget] = useState<RegistrationTeamMember | null>(null);

  const [activities, setActivities] = useState<RegistrationTeamActivity[]>([]);
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listRegistrationTeam();
  ...
  const refreshActivities = useCallback(async () => {
    ...
    try {
      setActivities(await listRegistrationTeamActivity());
    } catch {
      setActivities([]);
    } finally {
  ...
  const openEdit = (m: RegistrationTeamMember) => {
    setEditing(m);
    setForm({
      full_name: m.full_name,
      email: m.email,
      phone: m.phone,
      department: DEPARTMENTS.includes(m.department) ? m.department : DEPARTMENTS[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await updateRegistrationTeamMember(editing.id, {
        ...
        closeModal();
        await refresh();
        addToast('Member Updated', `${form.full_name}'s details updated successfully.`, 'success');
      } else {
        const res = await createRegistrationTeamMember({
          full_name: form.full_name,
          email: form.email,
          phone: form.phone,
          department: form.department,
        ...
        setForm(emptyForm);
        await refresh();
        addToast('Member Added', `${form.full_name} added to the Registration Team.`, 'success');
      }
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteRegistrationTeamMember(deleteConfirm.id, performer());
      await refresh();
      addToast('Member Deleted', `${deleteConfirm.full_name} was deleted from the Registration Team.`, 'success');
    } catch (err: any) {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      await setRegistrationTeamStatus(statusTarget.id, nextStatus, performer());
      await refresh();
      addToast(
  ...
  const openResetDialog = (m: RegistrationTeamMember) => {
    setResetTarget(m);
    setResetPassword('');
    setResetConfirm('');
    setResetErrors({});
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await resetRegistrationTeamPassword(resetTarget.id, performer(), resetPassword);
      await refresh();
      setResetTarget(null);
      addToast('Password Reset', `Password reset email sent to ${resetTarget.email}. Member must set a new password on next login.`, 'success');
    } catch (err: any) {
  ...
  const statusBadge = (m: RegistrationTeamMember) => (
  ...
  const passwordBadge = (m: RegistrationTeamMember) =>
    ...
    <div className="flex flex-col gap-6 select-none pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-violet-400 uppercase tracking-widest">Access Control & Registration Desk</span>
          <h2 className="text-xl sm:text-2xl font-extrabold font-display text-white">Registration Team Management ({members.length})</h2>
        </div>
        <button onClick={openAdd} className="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold transition-all shadow-lg shadow-violet-500/25 cursor-pointer flex items-center gap-2">
          ...
          <span>Add Registration Team Member</span>
        </button>
      </div>
      ...
      {tab === 'members' ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex flex-col gap-2">
              <span className="text-xs font-semibold text-emerald-300/70">Active Members</span>
              <span className="text-2xl font-extrabold text-white font-display">{activeCount}</span>
              <span className="text-[10px] text-emerald-400">Currently staffing the desk</span>
            </div>
            <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col gap-2">
              <span className="text-xs font-semibold text-amber-300/70">Inactive Members</span>
            ...
            <div className="p-5 rounded-2xl bg-violet-500/10 border border-violet-500/20 flex flex-col gap-2">
              <span className="text-xs font-semibold text-violet-300/70">Total Members</span>
              <span className="text-2xl font-extrabold text-white font-display">{members.length}</span>
              <span className="text-[10px] text-violet-400">Registration desk staff</span>
            </div>
            <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col gap-2">
                    ...
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan={10} className="px-5 py-12 text-center text-xs text-white/40">
                          No registration team members found.
                        </td>
                      </tr>
                    ) : (
                      filtered.map((m) => (
                        <tr key={m.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-1.5">
                              <button onClick={() => setViewTarget(m)} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/60 hover:text-white cursor-pointer" title="View Member"><Eye className="w-3.5 h-3.5" /></button>
                              <button onClick={() => openEdit(m)} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/70 hover:text-white cursor-pointer" title="Edit Member Details"><Edit className="w-3.5 h-3.5" /></button>
                              <button
                              ...
                              <button onClick={() => setDeleteConfirm(m)} className="p-2 rounded-lg bg-white/5 hover:bg-rose-500/20 text-white/40 hover:text-rose-400 cursor-pointer" title="Delete Member"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                          </td>
                        </tr>
                      ))
        ...
        <div className="rounded-3xl bg-zinc-950/60 border border-white/10 backdrop-blur-md overflow-hidden">
          <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-xs font-bold text-white">Registration Team Activity</span>
              ...
              activities.map((a) => (
                <div key={a.id} className="px-5 py-3.5 flex flex-col gap-1.5">
                  ...
                  <p className="text-[10px] text-white/40">
                    Member: <span className="text-white/60">{a.member_name}</span> · By: <span className="text-white/60">{a.performed_by_name}</span>
                  </p>
                </div>
              ))
            )}
              ...
              <div className="flex items-center justify-between gap-4 px-6 py-5 border-b border-white/10 bg-white/[0.02]">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    <h3 className="text-base font-bold font-display text-white tracking-tight truncate">{editing ? 'Edit Member' : 'Add Registration Team Member'}</h3>
                    <span className="text-[10px] text-white/40 tracking-widest uppercase">
                      {editing ? 'Update member details' : 'Create a registration desk account'}
                    </span>
                  </div>
                </div>
                ...
                {createdCredentials ? (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-4 text-center py-4">
                    ...
                    <div className="flex flex-col gap-1">
                      <h4 className="text-lg font-bold font-display text-white">Member added successfully!</h4>
                      <p className="text-xs text-white/50">The temporary password set above is ready for this member's first login.</p>
                    </div>
                  ...
                  <form id="rt-member-form" onSubmit={handleSubmit} className="flex flex-col gap-5 text-xs">
                    <FormSection icon={<User className="w-3.5 h-3.5" />} title="Personal Information" subtitle="Basic identity details of the desk member">
                    ...
                    <FormSection icon={<Building2 className="w-3.5 h-3.5" />} title="Department & Identity" subtitle="Where does this member belong?">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <FormSection icon={<ShieldCheck className="w-3.5 h-3.5" />} title="Account Settings" subtitle="Control access to the registration desk">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field label="Role">
                          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white/70">
                            <ShieldCheck className="w-3.5 h-3.5 text-violet-400" />
                            <span className="font-bold">Registration Team</span>
                          </div>
                        </Field>
          ...
          >
            ...
            >
              <div className="p-6 border-b border-white/10 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-white/40">Member ID · {viewTarget.user_id || viewTarget.id}</span>
                  </div>
                </div>
              ...
              <div className="p-6 flex flex-col gap-3 text-xs">
                ...
                <InfoRow icon={ShieldCheck} label="Role" value="Registration Team" />
            ...
            >
              ...
              <div className="p-6 flex flex-col gap-4 text-xs">
                <p className="text-white/60 leading-relaxed">
                  Enter a new temporary password for this member. A secure password reset email will also be sent to{' '}
                  <span className="text-white font-semibold">{resetTarget.email}</span> so they can set a new password on their next login.
                  The member will be required to change this password before accessing the Registration Desk.
                </p>
              ...
              <div className="px-6 py-4 border-t border-white/10 bg-white/[0.02] flex items-center justify-end gap-3">
                <button onClick={() => { if (!resetLoading) setResetTarget(null); }} className="px-4 py-2.5 rounded-xl text-white/60 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold transition-colors cursor-pointer">
                  Cancel
                </button>
                <button
                  onClick={handleResetPassword}
                  disabled={resetLoading}
                  className="px-6 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
                >
      ...
      <ConfirmationDialog
        open={!!deleteConfirm}
        title="Delete Member"
        message={`Are you sure you want to permanently delete "${deleteConfirm?.full_name}"? The member will no longer be able to log in and will be removed from the list.`}
        confirmLabel="Delete"
        variant="danger"
      ...
      <ConfirmationDialog
        ...
        message={
          statusTarget?.status === 'Active'
            ? `${statusTarget?.full_name} will be deactivated and will no longer be able to log in to the Registration Desk.`
            : `${statusTarget?.full_name} will be activated and able to log in to the Registration Desk.`
        }
