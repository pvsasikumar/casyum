 ...
 *
 * The QR code carries the participant's unique CASYUM id (`CAS-02`) — never
 * PII. Every lookup below resolves that id against the `casyum_id` field of
 * the `participants` collection (`participants where casyum_id == "CAS-02"`),
 * re-checks the workflow rules on
  ...
  try {
    ...
    if (profile.verificationStatus === 'Verified') {
      ...
      return {
        ...
        profile,
        message: profile.verifiedAt
          ? `Already Verified at ${new Date(profile.verifiedAt).toLocaleString()}.`
          : 'Already Verified.',
      };
    }
