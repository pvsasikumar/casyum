...
export interface CasyumFacultyActivity {
  id: string;
  log_id: string;
  member_id: string;
  member_name: string;
...
async function logActivity(entry: {
  ...
  const db = getDb();
  const logId = `cflog-${String(await nextSequence('casyum_faculty_logs'))}`;
  const timestamp = now();
  await setDoc(doc(db, 'casyum_faculty_logs', logId), {
    log_id: logId,
    ...entry,
    timestamp,
...
export async function createCasyumFacultyCoordinator(data: {
  ...
  const db = getDb();
  const user_id = `CFM-${String(await nextSequence('casyum_faculty'))}`;
...
export async function setCasyumFacultyStatus(
  id: string,
  status: 'Active' | 'Inactive',
...
export async function resetCasyumFacultyPassword(
  ...
  // manager is flagged to create a new password on next sign-in and a secure
  ...
  return {
    message: `A password reset email has been sent to ${existing.email}. The manager must set a new password on their next login.`,
  };
}

export async function listCasyumFacultyActivity(limitCount = 500): Promise<CasyumFacultyActivity[]> {
  const db = getDb();
  const snap = await getDocs(collection(db, 'casyum_faculty_logs'));
  return snap.docs
    .map((d) => {
      const data = d.data();
      return {
...
export interface PaymentReviewLogEntry {
  id: string;
  log_id: string;
...
export async function writePaymentReviewLog(entry: {
  registration_id: string;
  participant_id: string;
  participant_name: string;
  event_id: string;
  event_name: string;
  action: PaymentReviewLogEntry['action'];
  reviewer_id: string;
  reviewer_name: string;
  reason?: string;
}): Promise<void> {
  const db = getDb();
  const logId = `pvreview-${String(await nextSequence('payment_verification_logs'))}`;
  const timestamp = now();
  await setDoc(doc(db, 'payment_verification_logs', logId), {
    log_id: logId,
    ...entry,
    reason: entry.reason || '',
    timestamp,
...
export function subscribePaymentReviewLogs(
  onNext: (logs: PaymentReviewLogEntry[]) => void,
  onError?: (error: Error) => void
): () => void {
  const db = getDb();
  return onSnapshot(
    collection(db, 'payment_verification_logs'),
    (snapshot) => {
      const logs = snapshot.docs.map((d) => {
        ...
        return {
          ...
          action: (data.action || 'verified') as PaymentReviewLogEntry['action'],
          reviewer_id: data.reviewer_id || '',
          reviewer_name: data.reviewer_name || '',
          reason: data.reason || '',
          timestamp: data.timestamp || '',
          created_at: data.created_at || '',
        } as PaymentReviewLogEntry;
      });
