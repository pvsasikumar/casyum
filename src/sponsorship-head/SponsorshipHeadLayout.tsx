...
const PAGE_TITLES: Record<string, string> = {
  dashboard: 'Dashboard',
  enquiries: 'Sponsorship Enquiries',
  profile: 'Profile',
};

const NAV_ITEMS = [
  { to: '/sponsorship-head/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { to: '/sponsorship-head/enquiries', label: 'Enquiries', icon: Inbox },
  { to: '/sponsorship-head/profile', label: 'Profile', icon: User },
];
...
export const SponsorshipHeadLayout: React.FC = () => {
  const { user, isAuthenticated, isLoading, logout, enquiries } = useSponsorshipHead();
  ...
  const openCount = enquiries.filter((e) => !['Approved', 'Rejected', 'Closed'].includes(e.status)).length;

  useEffect(() => {
    document.title = `CASYUM Sponsorship — ${pageTitle}`;
  }, [pageTitle]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-violet-500/30 selection:text-violet-200">
      ...
      <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-violet-600 to-fuchsia-500 p-[1px] shadow-[0_0_15px_rgba(139,92,246,0.3)]">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold font-display text-white text-sm leading-none">
                CASYUM <span className="text-violet-400">Sponsorship</span>
              </span>
              <span className="text-[9px] text-white/40 tracking-widest uppercase mt-0.5">
                Sponsorship Head Portal
              </span>
            </div>
          </div>
          ...
          <nav className="hidden xl:flex items-center gap-1">
            {NAV_ITEMS.map((item) => (
              <NavLink key={item.to} to={item.to} className={navLinkClass}>
                ...
                {item.label === 'Enquiries' && openCount > 0 && (
                  <span className="px-1.5 py-0.5 rounded-full bg-violet-500/20 border border-violet-500/30 text-[9px] font-bold text-violet-300">
                    {openCount}
                  </span>
                )}
              </NavLink>
      ...
      <div className="border-b border-white/5 bg-black/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-2 text-[11px]">
          <span className="text-white/40">Sponsorship Head</span>
      ...
      {logoutDialogOpen && (
        <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-zinc-950 border border-white/20 rounded-3xl p-6 shadow-2xl flex flex-col gap-4">
            ...
            <p className="text-sm text-white/70">Are you sure you want to logout from the Sponsorship Head Portal?</p>
...
export default SponsorshipHeadLayout;
